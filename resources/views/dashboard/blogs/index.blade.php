                    ...
                    @forelse ($blogs as $blog)
                        <tr>
                            ...
                            <td>
                                ...
                                <h6 class="badge bg-@if($blog['status'] == '1')info @else()warning @endif">@if($blog->status == '1') Published @else Draft @endif</h6>&nbsp;
                                <form action="{{route('admin.blogs.status',['id'=>$blog->id])}}" method="POST" >
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="name" value="{{$blog->status}}">
                                    <button class="p-0" data-bs-toggle="tooltip" data-bs-placement="bottom" title="change status">
                                        <svg width="18" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">                                    
                                        <path d="M16.8397 20.1642V6.54639" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>                                    
                                        <path d="M20.9173 16.0681L16.8395 20.1648L12.7617 16.0681" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>                                    <path d="M6.91102 3.83276V17.4505" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>                                    
                                        <path d="M2.8335 7.92894L6.91127 3.83228L10.9891 7.92894" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>                                </svg>                                                                    
                                    </button>
                                </form>
                                </div>   
                              </td>
                            <td>
                                <div class="d-flex align-items-center mb-2">
                                    <a href="{{route('admin.blogs.edit', $blog->slug)}}" class="btn-outline-info m-1">                                        
